...
const EditBarber = () => {
  ...
  useEffect(() => {
    const fetchBarber = async () => {
      try {
        ...
        const barber = data.find(b => b._id === id);
        if (barber) {
          setFormData({
            name: barber.name,
            ...
            businessLicense: barber.businessLicense || ''
          });
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/barbers/${id}`, data, {
        headers: {
  ...
  if (fetching) return <div className="text-white text-center py-20">Loading barber details...</div>;
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 text-white pb-10">
      <div className="flex items-center space-x-4">
        ...
        <h1 className="text-2xl font-semibold">Edit Barber Account</h1>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 sm:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-2">Profile Photo (Leave empty to keep current)</label>
            <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-700 border-dashed rounded-lg bg-gray-800 hover:bg-gray-700/50 transition-colors">
              <div className="space-y-1 text-center">
                ...
                {photo && <p className="text-sm text-green-500 mt-2">New selected: {photo.name}</p>}
              </div>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-gray-800 flex justify-end">
            <button
              type="submit"
              disabled={loading}
              className="inline-flex justify-center py-3 px-6 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 transition-colors"
            >
              {loading ? 'Saving Changes...' : 'Update Barber Account'}
            </button>
          </div>
        </form>
      </div>
